...
module.exports = function personasRepository (models, Sequelize) {
  const { personas, pasajeros } = models;
  ...
  async function createOrUpdate (persona, t) {
    const cond = {
      where: {
        id: persona.id
      }
    ...
    if (item) {
      ...
      try {
        ...
        updated = await personas.update(persona, cond);
      } catch (e) {
        if (t) {
          t.rollback();
        }
    ...
    let result;
    try {
      result = await personas.create(persona, t ? { transaction: t } : {});
    } catch (e) {
      if (t) {
        t.rollback();
      }
