  ...
  return {
    Query: {
      usuarios: async (_, args, context) => {
        ...
        let lista = await Usuario.findAll(args, context.rol, context.id_entidad, context.id_operador);
    ...
    Mutation: {
      usuarioAdd: async (_, args, context) => {
        ...
        let item = await Usuario.createOrUpdate(args.usuario, context.rol, context.id_operador);
        return removeDots(item.data);
      },
